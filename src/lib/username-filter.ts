// Reserved system names and a profanity blocklist for usernames.
// Checks are case-insensitive and strip common leet-speak substitutions.
...
// Common profanity/slurs (normalized lowercase, no leet). Kept as a Set for O(1) lookup.
...
// Normalize a string: lowercase, collapse underscores/digits that substitute letters
...
export function validateUsername(raw: string): UsernameValidationResult {
  ...
  for (const word of BLOCKED_WORDS) {
    if (norm.includes(normalize(word))) {
      return {
        ok: false,
        reason: "That username contains a word that isn't allowed.",
      };
    }
  }
